...
<body>
	<c:set var="GraphView" value="${GraphView}"/>
				...
				<section>
					<div>
						<div class="order-button">
							<button type="button" class="recommended-order" onclick="location.href='${pageContext.request.contextPath }/ClassGraphView.cl'">추천순</button>
							<button type="button" class="latest-order" onclick="location.href='${pageContext.request.contextPath }/ClassLikeLatest.cl'">최신순</button>
						</div>
						<div class="class-wrap">
							<div class="classes-wrap">
								<c:choose>
									<c:when test="${GraphView != null and fn:length(GraphView) > 0}">
										<c:forEach var="classs" items="${GraphView}">
											<c:if test="${i%j == 0 }">
											<ul class="classes">
											</c:if>
